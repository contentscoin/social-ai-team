...
7. **인앱 렌더 엔진 — 프롬프트 md가 아니라 실물 (v0.9)** — 카드 인스펙터의 [🎨 비주얼 생성]으로 실제 PNG/MP4를 만듭니다.
   ...
   - **OpenCrab 팩 → 채널 카피 전략 (v0.17)** — 팩 검색 결과에서 팩을 **두 경로**로 로드할 수 있습니다: **[시각 팩]**은 기존처럼 이미지/영상 프롬프트 컴파일러용 전역 팩으로, **[채널 카피로]**는 그 팩을 현재 클라이언트의 선택 채널(threads/instagram/x/…) 카피 전략(`context/strategy/channel-<채널>-topexposure.md`)으로 저장합니다. 예를 들어 **Threads 상위노출 팩**(주제별 상위노출 글을 분석한 훅·포맷·타이밍 온톨로지)을 `threads`로 로드하면, copywriter가 Threads 글을 쓸 때 그 파일을 **best-performers급 리듬 레퍼런스**로 미러링합니다(threads-writer 스킬은 그대로, 라우팅은 copywriter 에이전트가 담당 — **배정 플랫폼 파일만** 읽어 다른 채널 오염 방지). 시각 컴파일러와 카피 경로가 분리되어, 카피용 팩이 이미지 프롬프트를 오염시키지 않습니다.
